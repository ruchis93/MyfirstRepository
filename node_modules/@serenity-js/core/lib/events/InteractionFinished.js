"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.InteractionFinished = void 0;
const model_1 = require("../model");
const ActivityFinished_1 = require("./ActivityFinished");
/**
 * @group Events
 */
class InteractionFinished extends ActivityFinished_1.ActivityFinished {
    static fromJSON(o) {
        return new InteractionFinished(model_1.CorrelationId.fromJSON(o.sceneId), model_1.CorrelationId.fromJSON(o.activityId), model_1.ActivityDetails.fromJSON(o.details), model_1.Outcome.fromJSON(o.outcome), model_1.Timestamp.fromJSON(o.timestamp));
    }
}
exports.InteractionFinished = InteractionFinished;
//# sourceMappingURL=InteractionFinished.js.map
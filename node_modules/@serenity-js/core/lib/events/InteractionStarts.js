"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.InteractionStarts = void 0;
const model_1 = require("../model");
const ActivityStarts_1 = require("./ActivityStarts");
/**
 * @group Events
 */
class InteractionStarts extends ActivityStarts_1.ActivityStarts {
    static fromJSON(o) {
        return new InteractionStarts(model_1.CorrelationId.fromJSON(o.sceneId), model_1.CorrelationId.fromJSON(o.activityId), model_1.ActivityDetails.fromJSON(o.details), model_1.Timestamp.fromJSON(o.timestamp));
    }
}
exports.InteractionStarts = InteractionStarts;
//# sourceMappingURL=InteractionStarts.js.map